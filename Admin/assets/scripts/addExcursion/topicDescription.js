...
function createTopicPlaceItem(name, photo_desc) {
  ...
  img.setAttribute("src", "assets/images/gallery-add.svg");
  ...
  closeIcon.setAttribute("src", "assets/images/close-square.svg");
...
appendTopicImage.addEventListener("click", () => {
  ...
  const newPlace = createTopicPlaceItem(
    `photo${lastElCount}`,
    `photo_desc${lastElCount}`,
  );
